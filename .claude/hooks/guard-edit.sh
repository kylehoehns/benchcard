...
# six chart pages are generated -- fix them by hand and the next
# `npm run charts` throws the fix away, silently, with a green suite.
set -uo pipefail
...
case "$path" in
  */app/*-player-basketball-rotation-chart.html)
    deny 'Blocked: the six roster-size chart pages are GENERATED by scripts/charts.mjs. An edit here survives until the next `npm run charts` and then vanishes with the suite still green. Edit scripts/charts.mjs and regenerate.'
    ;;
esac

 ...
 * exported, and a second, hand-typed copy of the 17 names would drift from it
...
test('the refusal lists the 17 selectable rows, one per line', () => {
  assert.equal(validNames.length, 17,
    `expected the 17 --only-able rows, got ${validNames.length}: ${JSON.stringify(validNames)}`);
